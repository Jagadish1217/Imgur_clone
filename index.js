...
let arr = [];
let c = 1;
function getData(pn = c, lmt = 30) {
  divv.innerHTML = "";
  fetch(
    `https://api.unsplash.com/photos/?client_id=6D-YM99mSzZNxet5G8SHtxzTPPAQnYXquaHQCYKZWf4&page=${pn}&_limit=${lmt}`
  )
    ...
    .then((data) => {
      ...
      arr.map((e) => {
        ...
        imageDiv.classList.add("cardImages");

        /* cardImages*/

        let image = document.createElement("img");
        image.src = e.urls.raw;

        /* Heading */

        let imageText = document.createElement("div");
        imageText.classList.add("ImagesText");
        let h6 = document.createElement("h6");
        h6.innerText =
          e.alt_description || e.description || "random clicks";
        imageText.appendChild(h6);
        let imageB = document.createElement("div");
        imageB.classList.add("imagesText");
        imageB.classList.add("imagesbot");
        imageB.innerHTML = `<div>
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <title>Upvote</title>
                            <path fill="#aaa" stroke="#ffffff" stroke-width="0" fill-rule="evenodd"
                                clip-rule="evenodd"
                    ...
                    <div>
                        <svg width="16" height="16" viewBox="0 0 16 16" class="PostCommentsIcon" fill="#85ADC2"
                            ...
                            <path fill="#aaa" stroke="#ffffff" stroke-width="0"
        ...
        imageDiv.appendChild(imageB);
        divv.appendChild(imageDiv);
      });
    });
}
